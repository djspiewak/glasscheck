...
mod linux {
    ...
    use glasscheck_core::{
        compare_images, AnchoredTextExpectation, CompareConfig, Point, PollOptions, Rect,
        RegionSpec, RelativeBounds, RgbaColor, Role, Selector, Size, TextAssertionConfig,
    };
    ...
    fn root_attachment_starts_window_offscreen_for_all_connected_displays(harness: GtkHarness) {
        ...
        if let Some(displays) = connected_display_rects(&display) {
            let frame =
                x11_window_frame(host.window()).expect("X11 toplevel frame should be readable");
